...
app.get("/test/scan", async (req, res) => {
	const { projects, invalidProjects } = await scanProjects(["./tests/test_projects"]);
	res.send(Array.from(projects.values()));
	//res.send(invalidProjects);
});

app.get("/test/validate", async (req, res) => {
	const parsed = await parseFile("./tests/test_projects/factorio/manipanel.toml");
